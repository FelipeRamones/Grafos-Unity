...
public class q1Script : MonoBehaviour {
	...
	public void q1StateEnter(string text, int count){
			...
			if (text.ToCharArray () [0] == 'a' || text.ToCharArray () [0] == 'b') {
				...
				if (this.count <= 0) {
					...
					StartCoroutine (GameObject.Find ("GameController").GetComponent<GameControllerScript> ().waitASecondQ1 ());
				}
		} else {
			nodeColor = GameObject.Find ("GameController").GetComponent<GameControllerScript> ().failureColor;
			this.GetComponent<SpriteRenderer> ().color = nodeColor;
	...
	private void pointToNextState(){
		GameObject.Find ("q2").GetComponent<q2Script> ().q2StateEnter (this.text, this.count);
	}
}
